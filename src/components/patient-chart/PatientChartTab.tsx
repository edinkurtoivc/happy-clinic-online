...
export default function PatientChartTab({ patient }: { patient: any }) {
  ...
  const storageKey = useMemo(() => `clinical-${patient?.id}`, [patient?.id]);

  useEffect(() => {
    const load = async () => {
      try {
        const saved = await dataStorageService.getPatientClinicalData(String(patient?.id || ""));
        setClinical(saved);
      } catch {}
    };
    if (patient?.id) load();
  ...
  const persist = async (next: ClinicalData, actionDesc: string) => {
    setClinical(next);
    await dataStorageService.savePatientClinicalData(String(patient?.id || ""), next);
    try {
  ...
  const addHistoryEntry = (type: string, doctor?: string) => {
    try {
      const key = `patient-history-${patient?.id}`;
      const existing = JSON.parse(localStorage.getItem(key) || '[]');
      existing.unshift({ id: Date.now(), patientId: Number(patient?.id), date: new Date().toISOString().slice(0,10), type, doctor: doctor || '—' });
  ...
  return (
    <div className="space-y-4">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="vitals" className="space-y-4" id="section-vitals">
          <VitalSignsTab items={clinical.vitalSigns || []} onChange={(next) => persist({ ...clinical, vitalSigns: next }, 'vitalni znakovi ažurirani')} />
        </TabsContent>

        <TabsContent value="problems" className="space-y-4" id="section-problems">
          <Card className="p-4">
        ...
        <TabsContent value="allergies" className="space-y-4" id="section-allergies">
          <Card className="p-4">
        ...
        <TabsContent value="medications" className="space-y-4" id="section-medications">
          <Card className="p-4">
            <div className="grid gap-2 md:grid-cols-5">
