...
export default function MedicalReportVersions({ 
  ...
  const [selectedVersion, setSelectedVersion] = useState<MedicalReportVersion>(sorted[0]);
  ...
  useEffect(() => {
    if (sorted[0] && (!selectedVersion || !versions.find(v => v.id === selectedVersion.id))) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            {sorted.map((version, index) => (
              ...
              >
                ...
                <div>
                  <p className="font-medium">Verzija {version.version}</p>
                  <p className="text-xs text-muted-foreground">
                    {formatDate(version.createdAt)}
                  </p>
                  <p className="text-xs">{version.status === 'final' ? 'Final' : 'Nacrt'} • {version.createdBy}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="border rounded-md p-4 md:col-span-2">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h3 className="font-medium">Verzija {selectedVersion.version}</h3>
              <p className="text-sm text-muted-foreground">
                Kreirano {formatDate(selectedVersion.createdAt)} • {selectedVersion.status === 'final' ? 'Final' : 'Nacrt'} • {selectedVersion.createdBy}
              </p>
            </div>
